    ...
    def run(self):
        ...
        while self.Flag:
            ...
            if USEPAPER:
                # hand_pos, hand_normal, color_img = self.model.get_handpos(color_img)
                # print("hand_pos:", hand_pos)
                imgh, imgw, imgc = color_img.shape
                w, h = 5, 4
                ...
                if ret == True:
                    ...
                    cv2.drawChessboardCorners(color_img, (w, h), corners, ret)
                    ...
                    cp0 = [int(corners[0][0][0]), int(corners[0][0][1])]
                    p0 = np.array(self.model.ca.color2point(cp0))
                    hand_pos = p0
                    cp15 = [int(corners[15][0][0]), int(corners[15][0][1])]
                    p15 = np.array(self.model.ca.color2point(cp15))
                    hand_normal = p0 - p15
